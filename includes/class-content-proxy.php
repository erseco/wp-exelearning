...
class ExeLearning_Content_Proxy {
	...
	public function serve_content( $request ) {
		...
		// For HTML files, inject base tag to fix relative URLs.
		if ( 'html' === $extension || 'htm' === $extension ) {
			$this->serve_html_with_base_tag( $full_path, $hash, $mime_type, $file );
			exit;
		}

		// For CSS files, rewrite url() references when using query string URLs.
		$base_url = rest_url( 'exelearning/v1/content/' . $hash . '/' );
		if ( 'css' === $extension && strpos( $base_url, '?' ) !== false ) {
	...
	private function serve_html_with_base_tag( $full_path, $hash, $mime_type, $file_path = '' ) {
		...
		// Build the base URL for this content.
		// Use rest_url which handles both pretty permalinks and query string formats.
		$base_url = rest_url( 'exelearning/v1/content/' . $hash . '/' );

		// Check if we're using query string format (no pretty permalinks).
		// In that case, base tag won't work, so we need to rewrite URLs.
		if ( strpos( $base_url, '?' ) !== false ) {
			$html = $this->rewrite_relative_urls( $html, $hash, $file_path );
		} else {
			// Pretty permalinks enabled - just add base tag.
			...
			if ( preg_match( '/<head[^>]*>/i', $html, $matches, PREG_OFFSET_CAPTURE ) ) {
				...
				$html       = substr( $html, 0, $insert_pos ) . $base_tag . substr( $html, $insert_pos );
			} else {
				$html = $base_tag . $html;
			}
		}
