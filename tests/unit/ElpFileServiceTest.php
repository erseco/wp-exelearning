...
class ElpFileServiceTest extends WP_UnitTestCase {
	...
	public static function set_up_before_class() {
		...
		self::$test_elp_v2 = wp_tempnam( 'test_v2.elp' );
...
<package xmlns="http://www.imsglobal.org/xsd/imscp_v1p1">
    <odeProperties>
        <entry key="pp_title">Test V3 Title</entry>
        <entry key="pp_description">Test V3 Description</entry>
        <entry key="pp_author">Test Author</entry>
    </odeProperties>
</package>';
	...
	private static function create_test_elp_v2( $path ) {
		...
		// V2 files use contentv3.xml with pickle format.
		$content_xml = '<?xml version="1.0" encoding="UTF-8"?>
<pickle>
	<dictionary>
	...
	/**
	 * Test convert_v2_to_v3 method exists and is private.
	 */
	public function test_convert_v2_to_v3_exists() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'convert_v2_to_v3' );

		$this->assertTrue( $method->isPrivate() );
	}

	/**
	 * Test adjust_data_to_v3 method exists and is private.
	 */
	public function test_adjust_data_to_v3_exists() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'adjust_data_to_v3' );

		$this->assertTrue( $method->isPrivate() );
	}

	/**
	 * Test adjust_data_to_v3 returns array.
	 */
	public function test_adjust_data_to_v3_returns_array() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'adjust_data_to_v3' );
	...
	public function test_validate_elp_file_invalid_zip() {
		// Create a temporary file that is not a valid zip.
		$temp_file = wp_tempnam( 'notazip.elpx' );
		file_put_contents( $temp_file, 'PK' . str_repeat( 'x', 100 ) ); // Fake zip header.
	...
	/**
	 * Test validate_elp_file with valid v2 file returns converted status.
	 */
	public function test_validate_elp_file_v2_converted_status() {
	...
	/**
	 * Test validate_elp_file with v2 file returns data array.
	 */
	public function test_validate_elp_file_v2_returns_data() {
		$result = $this->service->validate_elp_file( self::$test_elp_v2 );
	...
	/**
	 * Test convert_v2_to_v3 with valid file.
	 */
	public function test_convert_v2_to_v3_with_valid_file() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'convert_v2_to_v3' );
		$method->setAccessible( true );

		$result = $method->invoke( $this->service, self::$test_elp_v2 );

		$this->assertIsArray( $result );
	}

	/**
	 * Test convert_v2_to_v3 with nonexistent file returns WP_Error.
	 */
	public function test_convert_v2_to_v3_with_nonexistent_file() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'convert_v2_to_v3' );
		$method->setAccessible( true );

		$result = $method->invoke( $this->service, '/nonexistent/path.elp' );

		$this->assertInstanceOf( WP_Error::class, $result );
		$this->assertEquals( 'conversion_error', $result->get_error_code() );
	}

	/**
	 * Test convert_v2_to_v3 with invalid zip returns WP_Error.
	 */
	public function test_convert_v2_to_v3_with_invalid_zip() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'convert_v2_to_v3' );
		$method->setAccessible( true );

		$temp_file = wp_tempnam( 'invalid.elp' );
		file_put_contents( $temp_file, 'not a zip file' );

		$result = $method->invoke( $this->service, $temp_file );
	...
	/**
	 * Test validate_elp_file returns correct version for v3.
	 */
	public function test_validate_elp_file_v3_correct_version() {
		$result = $this->service->validate_elp_file( self::$test_elp_v3 );
	...
	/**
	 * Test adjust_data_to_v3 preserves nested arrays.
	 */
	public function test_adjust_data_to_v3_preserves_nested() {
		$method = new ReflectionMethod( ExeLearning_Elp_File_Service::class, 'adjust_data_to_v3' );
		$method->setAccessible( true );
