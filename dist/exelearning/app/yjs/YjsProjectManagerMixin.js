...
const YjsProjectManagerMixin = {
  ...
  applyMixin(projectManager) {
    ...
    projectManager.enableYjsMode = async function (projectId, authToken, options = {}) {
      console.log('[ProjectManager] Enabling Yjs mode for project:', projectId);
      ...
      console.log('[ProjectManager] Yjs mode enabled');
      ...
      if (this.app?.interface?.odeTitleElement) {
        ...
        console.log('[ProjectManager] Title element updated from Yjs');
      }

      // Initialize theme binding now that Yjs is ready
      if (this.app?.themes) {
        this.app.themes.initYjsBinding();
        console.log('[ProjectManager] Theme binding initialized from Yjs');
      }
    ...
    projectManager._setupTinyMCEHook = function () {
      ...
      if (window.$exeTinyMCE) {
        window.$exeTinyMCE.onEditorInit = function (editor) {
          ...
          if (ids) {
            console.log('[YjsHook] Binding editor to Yjs:', ids);
            self.bindEditorToYjs(editor, ids.pageId, ids.blockId, ids.componentId);
          } else {
            console.log('[YjsHook] Could not extract IDs for editor:', editor.id);
          }
        };
      }
    };
    ...
    projectManager._extractIdsFromEditor = function (editor) {
      try {
        ...
        if (!activePageId) {
          console.log('[YjsHook] No active page ID found');
          return null;
        }
        ...
        if (!componentId) {
          console.log('[YjsHook] No component ID found');
          return null;
        }
    ...
    projectManager.disableYjsMode = async function () {
      ...
      console.log('[ProjectManager] Disabling Yjs mode');
      ...
      console.log('[ProjectManager] Yjs mode disabled');
    };
    ...
    projectManager.save = async function (...args) {
      if (this._yjsEnabled && this._yjsBridge) {
        console.log('[ProjectManager] Saving via Yjs...');
    ...
    projectManager.generateIntervalAutosave = function (...args) {
      if (this._yjsEnabled) {
        console.log('[ProjectManager] Autosave skipped - using Yjs auto-sync');
        return;
      }
    ...
    projectManager.deletePageViaYjs = function (pageId) {
      if (this._yjsEnabled && this._yjsBridge) {
        this._yjsBridge.deletePage(pageId);
        return true;
      }
      return false;
    };
    ...
    projectManager.renamePageViaYjs = function (pageId, newName) {
      if (this._yjsEnabled && this._yjsBridge) {
        ...
        console.log('[YjsProjectManager] Renamed page via Yjs:', pageId, newName);
        return true;
      }
      return false;
    };
    ...
    projectManager.importConvertedStructure = async function (structure, assets, options = {}) {
      ...
      console.log(`[ProjectManager] Importing converted structure: ${structure?.pages?.length || 0} pages, ${assets?.length || 0} assets`);
      ...
      if (assetManager && assets && assets.length > 0) {
        ...
        console.log(`[ProjectManager] Imported ${stats.assets} assets`);
      }
      ...
      if (structure.pages && Array.isArray(structure.pages)) {
        ...
        if (hasChildren) {
          ...
          console.log(`[ProjectManager] Flattened ${structure.pages.length} pages to ${flatPages.length} flat pages`);
        } else {
      ...
      console.log(`[ProjectManager] Import complete:`, stats);
      return stats;
    };
    ...
    console.log('[YjsProjectManagerMixin] Mixin applied to projectManager');
  },
