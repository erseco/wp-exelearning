...
class YjsLockManager {
  ...
  constructor(ydoc, awareness = null) {
    ...
    console.log('[YjsLockManager] Initialized');
  }
  ...
  requestLock(componentId) {
    ...
    if (existingLock && existingLock.clientId !== this.getClientId()) {
      ...
      if (this.isLockStale(existingLock)) {
        console.log(`[YjsLockManager] Overriding stale lock on ${componentId}`);
      } else {
        console.log(`[YjsLockManager] Cannot lock ${componentId}: locked by ${existingLock.user.name}`);
        return false;
      }
    }
    ...
    console.log(`[YjsLockManager] Acquired lock on ${componentId}`);
    return true;
  }
  ...
  releaseLock(componentId) {
    ...
    if (lock && lock.clientId === this.getClientId()) {
      ...
      console.log(`[YjsLockManager] Released lock on ${componentId}`);
    }
  }
  ...
  releaseAllMyLocks() {
    ...
    this.myLocks.clear();
    console.log('[YjsLockManager] Released all my locks');
  }
  ...
  handleAwarenessChange(removedClientIds) {
    ...
    this.locks.forEach((lock, componentId) => {
      if (removedSet.has(lock.clientId)) {
        console.log(`[YjsLockManager] Auto-releasing lock on ${componentId} (user disconnected)`);
        this.locks.delete(componentId);
      }
    });
  }
  ...
  cleanupStaleLocks() {
    ...
    if (cleaned > 0) {
      console.log(`[YjsLockManager] Cleaned up ${cleaned} stale locks`);
    }
  }
  ...
  destroy() {
    ...
    console.log('[YjsLockManager] Destroyed');
  }
}
