...
export default class modalOpenUserOdeFiles extends Modal {
    ...
    async openUserOdeFilesEvent(projectUuid) {
        ...
        console.log(`[OpenProject] Opening project: ${projectUuid}`);
    ...
    async deleteOdeFileEvent(projectUuid) {
        try {
            ...
            const response = await fetch(`${apiUrl}/api/v2/projects/uuid/${projectUuid}`, {
                method: 'DELETE',
                headers: {
    ...
    async duplicateOdeFileEvent(projectUuid) {
        try {
            ...
            const response = await fetch(`${apiUrl}/api/v2/projects/uuid/${projectUuid}/duplicate`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...(authToken ? { 'Authorization': `Bearer ${authToken}` } : {}),
    ...
    async massiveDeleteOdeFileEvent(projectUuids) {
        try {
            ...
            for (const uuid of projectUuids) {
                await fetch(`${apiUrl}/api/v2/projects/uuid/${uuid}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        ...(authToken ? { 'Authorization': `Bearer ${authToken}` } : {}),
    ...
    async largeFilesUpload(
        ...
        if (!isImportIdevices && !isImportProperties) {
            try {
                ...
                console.log('[OpenFile] Project data received:', projectData);
                ...
                console.log(`[OpenFile] Created project ${projectUuid}, processing ELP directly in memory...`);
                ...
                try {
                    ...
                    console.log(`[OpenFile] Yjs reinitialized for project ${projectUuid}`);
                    ...
                    console.log(`[OpenFile] Import complete:`, stats);
                    ...
                    console.log(`[OpenFile] Successfully opened ${odeFileName}`);
                } catch (err) {
                ...
                console.log('[OpenFile] Falling back to legacy upload flow...');
            }
        }
        ...
        if (response.responseMessage == 'OK') {
            ...
            if (!isImportIdevices) {
                ...
                if (response.projectUuid && response.elpImportPath) {
                    console.log(`[OpenFile] Redirecting to Yjs project: ${response.projectUuid}`);
                    console.log(`[OpenFile] Import path: ${response.elpImportPath}`);
