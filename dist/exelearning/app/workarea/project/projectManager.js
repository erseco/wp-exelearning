...
export default class projectManager {
    ...
    async initializeYjs() {
        ...
        if (!window.YjsLoader && !window.YjsModules) {
            console.log('[ProjectManager] Yjs not available, using legacy mode');
            return;
        }
        ...
        try {
            ...
            if (window.YjsLoader && !window.YjsModules?.YjsProjectBridge) {
                console.log('[ProjectManager] Loading Yjs modules...');
            ...
            if (localOnlyMode) {
                console.log('[ProjectManager] Yjs local-only mode (offline installation)');
            } else {
                console.log('[ProjectManager] Yjs collaborative mode (WebSocket + IndexedDB)');
            }
            ...
            if (this.enableYjsMode) {
                console.log('[ProjectManager] Enabling Yjs mode for project:', projectId, isNewProject ? '(new)' : '(existing)');
                await this.enableYjsMode(projectId, authToken, {
                    ...
                    isNewProject: isNewProject,  // Skip sync wait only for new projects
                });
                console.log('[ProjectManager] Yjs collaborative editing enabled');
    ...
    async reinitializeWithProject(projectUuid, options = {}) {
        console.log('[ProjectManager] Reinitializing with new project:', projectUuid, options);

        // Disconnect existing Yjs connection if present
        if (this._yjsBridge) {
            console.log('[ProjectManager] Disconnecting existing Yjs bridge...');
            try {
        ...
        // Reset project state (now projectId is already set)
        this.resetProject();

        // Reinitialize Yjs with the new project
        ...
        console.log('[ProjectManager] Yjs reinitialized for project:', projectUuid);
    }
    ...
    async importElpDirectly(file, options = {}) {
        console.log('[ProjectManager] Importing ELP directly from memory:', file.name);
        ...
        console.log('[ProjectManager] Direct import complete:', stats);

        // Save to server
        try {
            await this._yjsBridge.documentManager.saveToServer();
            console.log('[ProjectManager] Document saved to server after direct import');
        } catch (saveError) {
    ...
    async refreshAfterDirectImport() {
        console.log('[ProjectManager] Refreshing UI after direct import...');
        ...
        if (this.structure) {
            ...
            if (this.structure.isYjsEnabled && this.structure.isYjsEnabled()) {
                ...
                console.log('[ProjectManager] Structure data from Yjs:', structureData?.length, 'pages');
        ...
        console.log('[ProjectManager] UI refreshed after direct import');
    }
    ...
    async checkAndImportElp() {
        ...
        if (!importPath) {
            console.log('[ProjectManager] No import parameter found');
            return;
        }

        console.log('[ProjectManager] Import parameter detected:', importPath);
        ...
        try {
            ...
            console.log('[ProjectManager] ELP file fetched, size:', file.size);

            // Use centralized import method (handles asset announcement)
            const stats = await this.importFromElpxViaYjs(file);

            console.log('[ProjectManager] ELP import complete:', stats);

            // Save to server so other users can access the document
            try {
                await this._yjsBridge.documentManager.saveToServer();
                console.log('[ProjectManager] Document saved to server after import');
            } catch (saveError) {
            ...
            if (stats.pages > 0) {
                console.log(`[ProjectManager] Successfully imported ${stats.pages} pages, ${stats.blocks} blocks, ${stats.components} components`);
            }
        } catch (error) {
     ...
     */
    resetProject() {
        console.log('[ProjectManager] Resetting project - clearing all previous state');
        ...
        if (this._yjsBridge?.clearNavigation) {
            console.log('[ProjectManager] Clearing Yjs navigation...');
        ...
        if (navTree) {
            console.log('[ProjectManager] Clearing navigation tree DOM...');
            navTree.innerHTML = '';
        }
        ...
        if (contentArea) {
            console.log('[ProjectManager] Clearing content area...');
        ...
        if (idevicePanels) {
            console.log('[ProjectManager] Clearing iDevice panels...');
        ...
        console.log('[ProjectManager] ✓ Project reset complete - ready for new content');
    }
    ...
    */
    ...
    async loadCurrentProject() {
        ...
        console.log(`[ProjectManager] Using project ID from URL: ${urlProjectId}`);
            ...
            if (this._yjsEnabled && this._yjsBridge) {
                ...
                if (documentManager) {
                    ...
                    if (result.success) {
                        ...
                        console.log('[ProjectManager] Yjs document saved:', result.message);
                    } else {
