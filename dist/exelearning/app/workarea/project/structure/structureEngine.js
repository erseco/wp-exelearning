...
export default class structureEngine {
    ...
    async getOdeStructure() {
        ...
        if (this.isYjsEnabled() && this.project._yjsBridge) {
            ...
            if (forceImport) {
                console.log('[StructureEngine] Force import flag set - clearing Yjs and fetching from API...');
                ...
                if (apiData && apiData.length > 0) {
                    ...
                    console.log('[StructureEngine] Force imported', apiData.length, 'pages into Yjs');
                }
            ...
            if (navigation && navigation.length > 0) {
                console.log('[StructureEngine] Loading structure from Yjs (has data)');
                return this.getStructureFromYjs();
            }

            // Yjs is empty - fetch from API and import
            console.log('[StructureEngine] Yjs empty, fetching from API and importing...');
            const apiData = await this.fetchStructureFromApi();

            if (apiData && apiData.length > 0) {
                // Import API data into Yjs
                bridge.importStructure(apiData);
                console.log('[StructureEngine] Imported', apiData.length, 'pages into Yjs');
            }
    ...
    getStructureFromYjs() {
        ...
        console.log('[StructureEngine] Loaded', structureData.length, 'pages from Yjs');
    ...
    subscribeToYjsChanges() {
        ...
        this.project._yjsBridge.onStructureChange(async (events, transaction) => {
            ...
            if (!isRemote) {
                console.log('[StructureEngine] Skipping local change (local:', transaction?.local, ')');
                return;
            }

            console.log('[StructureEngine] Yjs structure changed (remote), reloading...');
            ...
            if (this.menuStructureCompose) {
                ...
                if (pageToReload) {
                    ...
                    if (pageElement) {
                        console.log('[StructureEngine] Forcing page content reload for remote changes');
        ...
        // Load initial structure from Yjs immediately
        const structureData = this.getStructureFromYjs();
        if (structureData && structureData.length > 0) {
            console.log(`[StructureEngine] Loading initial ${structureData.length} pages from Yjs`);
            this.processStructureData(structureData);
        ...
        console.log('[StructureEngine] Subscribed to Yjs changes');
    }
    ...
    async resetStructureData(idSelect) {
        console.log('[StructureEngine] resetStructureData called with idSelect:', idSelect);
        // In Yjs mode, fully reload structure from Yjs document
        if (this.isYjsEnabled()) {
            const structureData = this.getStructureFromYjs();
            this.processStructureData(structureData);
            this.openNode(idSelect);
            console.log('[StructureEngine] About to call reloadStructureMenu with:', idSelect);
            await this.reloadStructureMenu(idSelect);
            console.log('[StructureEngine] reloadStructureMenu completed');
            return;
        }
    ...
    async reloadStructureMenu(idToSelect) {
        console.log('[StructureEngine] reloadStructureMenu called with:', idToSelect);
        this.menuStructureCompose.compose();
        this.menuStructureBehaviour.behaviour(false);
        if (idToSelect) {
            console.log('[StructureEngine] Calling selectNode with:', idToSelect);
            await this.selectNode(idToSelect);
            console.log('[StructureEngine] selectNode completed');
        } else {
            await this.selectFirst();
        }
    }
     ...
     */
    setTitleToNodeRoot() {
        // Preserve current selection - don't navigate users when title changes
        // Guard: Ensure nodeSelected is a DOM element before calling getAttribute
        const nodeSelected = this.menuStructureBehaviour?.nodeSelected;
        const selectedNavId = (nodeSelected && typeof nodeSelected.getAttribute === 'function')
            ? nodeSelected.getAttribute('nav-id')
    ...
    createNodeAndReload(parentId, title) {
        ...
        console.log('[StructureEngine] createNodeAndReload called, parentId:', parentId, 'title:', title);
        this.createNode(parentId, title).then((response) => {
            console.log('[StructureEngine] createNode response:', response);
            if (response && response.responseMessage == 'OK') {
                console.log('[StructureEngine] Calling resetStructureData with ID:', response.odeNavStructureSyncId);
     ...
     */
    async selectNode(id) {
        console.log('[StructureEngine] selectNode called with id:', id);
        const element = this.getNodeElement(id);
        console.log('[StructureEngine] getNodeElement returned:', element ? 'FOUND' : 'NOT FOUND', element);
        if (element) {
            console.log('[StructureEngine] Calling menuStructureBehaviour.selectNode...');
            await this.menuStructureBehaviour.selectNode(element);
            console.log('[StructureEngine] menuStructureBehaviour.selectNode completed');
        } else {
