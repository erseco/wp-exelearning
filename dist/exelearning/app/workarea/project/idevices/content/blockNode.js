...
export default class IdeviceBlockNode {
    ...
    loadPropertiesFromYjs() {
        ...
        if (yjsProperties) {
            ...
            console.log('[BlockNode] Loaded properties from Yjs:', this.blockId, yjsProperties);
        }
    }
    ...
    */
    ...
    async moveToPageViaYjs(targetPageId) {
        try {
            ...
            if (!success && this.id && this.id !== this.blockId) {
                console.log('[BlockNode] Retrying with id:', this.id);
    ...
    async deleteBlockViaYjs() {
        try {
            ...
            for (const blockIdToTry of idsToTry) {
                console.log('[BlockNode] Trying to delete block with id:', blockIdToTry);
                success = structureBinding.deleteBlock(this.pageId, blockIdToTry);
                if (success) {
                    console.log('[BlockNode] Block deleted via Yjs with id:', blockIdToTry);
                    break;
                }
            }
    ...
    async reorderViaYjs() {
        try {
            ...
            if (!success && this.id && this.id !== this.blockId) {
                console.log('[BlockNode] Retrying reorder with id:', this.id);
