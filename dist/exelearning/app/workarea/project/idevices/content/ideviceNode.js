...
export default class IdeviceNode {
    ...
    loadPropertiesFromYjs() {
        ...
        if (yjsProperties) {
            ...
            console.log('[IdeviceNode] Loaded properties from Yjs:', this.odeIdeviceId, yjsProperties);
        }
    }
    ...
    */
    ...
    async ideviceInitExport() {
        ...
        if (this.isLockedByOtherUser()) {
            ...
            if (isJsonType && hasNoContent) {
                console.log(`[IdeviceNode] Remote JSON iDevice ${this.odeIdeviceId} locked without content, showing placeholder`);
                return this.showLockedPlaceholder();
            }

            // For HTML iDevices without content, also show placeholder
            if (!isJsonType && hasNoContent) {
                console.log(`[IdeviceNode] Remote HTML iDevice ${this.odeIdeviceId} locked without content, showing placeholder`);
                return this.showLockedPlaceholder();
            }
        }
    ...
    showLockedPlaceholder() {
        ...
        console.log(`[IdeviceNode] Showing locked placeholder for ${this.odeIdeviceId}, locked by ${userName}`);
        ...
        try {
            ...
            for (const componentId of componentIds) {
                for (const targetBlockId of blockIds) {
                    ...
                    if (success) {
                        console.log('[IdeviceNode] Move to block succeeded with compId:', componentId, 'blockId:', targetBlockId);
                        break;
                    }
                }
                if (success) break;
    ...
    async moveToPageViaYjs(targetPageId) {
        try {
            ...
            for (const componentId of componentIds) {
                ...
                if (result) {
                    console.log('[IdeviceNode] Move to page succeeded with compId:', componentId);
                    break;
                }
            }
    ...
    async reorderViaYjs() {
        try {
            ...
            for (const componentId of idsToTry) {
                ...
                if (success) {
                    console.log('[IdeviceNode] Reorder succeeded with id:', componentId);
                    break;
                }
            }
    ...
    async deleteViaYjs() {
        try {
            ...
            if (success) {
                console.log('[IdeviceNode] Deleted component via Yjs:', this.id);
                return { responseMessage: 'OK' };
            } else {
