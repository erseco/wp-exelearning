    ...
    */
    ...
    async openClientPreview() {
        ...
        try {
            ...
            // Generate preview
            console.log('[NavbarFile] Starting client-side preview...');
            ...
            if (result.success) {
                ...
                console.log('[NavbarFile] Client-side preview opened successfully');
            } else {
                    ...
                    input.addEventListener('change', async () => {
                        ...
                        try {
                            ...
                            if (
                                ...
                                console.log('[NavbarFile] Importing via Yjs (client-side):', file.name);
                                ...
                                console.log('[NavbarFile] Yjs import complete:', stats);
    ...
    async createSession(params) {
        ...
        if (eXeLearning.app.project?._yjsEnabled &&
            ...
            console.log('[NavbarFile] Creating new project in Yjs mode');
            try {
                ...
                console.log('[NavbarFile] New project created:', projectUuid);

                // Clear beforeunload handler
                window.onbeforeunload = null;

                // Reinitialize Yjs with the new project (no page reload, skip sync wait)
                await eXeLearning.app.project.reinitializeWithProject(projectUuid, { isNewProject: true });

                // Update URL without reloading page (reuse basePath from above)
                window.history.pushState({}, '', `${basePath}/workarea?project=${projectUuid}`);

                // Trigger structure load to notify observers about the new Home page
                if (eXeLearning.app.project._yjsBridge?.triggerInitialStructureLoad) {
                    eXeLearning.app.project._yjsBridge.triggerInitialStructureLoad();
                }
    ...
    async downloadProjectViaYjs() {
        ...
        try {
            ...
            const projectTitle = eXeLearning.app.project?.properties?.properties?.pp_title?.value
                                || 'project';
            ...
            console.log('[NavbarFile] Project exported via Yjs:', filename);
    ...
    async exportViaYjs(format, fallbackApiFormat) {
        ...
        if (!supportedFormats.includes(normalizedFormat)) {
            console.log(`[NavbarFile] Format ${format} not yet supported client-side, using server`);
            return false;
        }
        ...
        try {
            ...
            // Export
            console.log(`[NavbarFile] Starting client-side ${format} export...`);
            const result = await exporter.export();

            if (result.success) {
                toast.toastBody.innerHTML = _('The project has been exported.');
                console.log(`[NavbarFile] Client-side export complete: ${result.filename}`);
            } else {
    ...
    async checkIfLegacyElpFormat(file) {
        try {
            ...
            if (rootTag === 'instance' || rootTag === 'dictionary') {
                console.log('[NavbarFile] Detected Python pickle format (legacy)');
                return true;
            }
