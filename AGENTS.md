...
- Post meta: `_exelearning_title`, `_description`, `_license`, `_language`,
  `_resource_type`, `_version`, `_extracted` (SHA1 of the extraction folder),
  `_has_preview` (`1` when `index.html` exists), `_obsolete_hash` (redirect aliases).
...
- Archive processing, styles and content delivery go through the existing
  ...
  Package HTML is untrusted author content served on the site origin: never
  frame it with `allow-same-origin` in wp-admin. The only exception is the
  dev-only `EXELEARNING_UNSAFE_LEGACY_IFRAME` constant, set by the Playground
  blueprint because its Service Worker cannot serve opaque-origin subframes.
